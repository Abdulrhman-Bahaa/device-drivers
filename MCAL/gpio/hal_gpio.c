 ...
 *             functions
 * @date       2023-12-06
 * @details    This source file contains implementations for the gpio module
 *             functions
 ******************************************************************************
*/
#include "hal_gpio.h"
...
volatile uint8* tris_register[PORT_MAX_NUMBER] = {&TRISA,&TRISB,&TRISC,&TRISD,&TRISE};
volatile uint8* port_register[PORT_MAX_NUMBER] = {&PORTA,&PORTB,&PORTC,&PORTD,&PORTE};

/* Functions Implementations -------------------------------------------------*/
Std_ReturnType hal_gpio_pin_direction_initialize(const pin_config_t* pin_config){
    Std_ReturnType ret = E_OK;
    uint8 bit_posn = pin_config->pin;
    if((NULL == pin_config) || (bit_posn > PORT_PIN_MAX_NUMBER - 1)) {
        ret = E_NOT_OK;
    } 
    else {
        volatile uint8* reg_addr = tris_register[pin_config->port];
...
Std_ReturnType hal_gpio_pin_logic_write(const pin_config_t* pin_config, logic_t logic) {
    ...
    if((NULL == pin_config) || (bit_posn > PORT_PIN_MAX_NUMBER - 1)) {
        ...
        volatile uint8* reg_addr = port_register[pin_config->port];
        switch(logic) {
            case HIGH :
                SET_BIT(reg_addr,bit_posn);
                break;

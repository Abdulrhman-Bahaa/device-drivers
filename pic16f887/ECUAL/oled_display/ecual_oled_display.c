...
//        ret |= ecual_oled_display_cursor_set(oled_display, page, column);
        for (uint8_t page_num = 0; page_num < round_up(height / 8.0); page_num++) {
            ...
            for (uint8_t bytes_number = 0; bytes_number < width; bytes_number++) {
                    index = ((page_num * width) + bytes_number) + (char_to_display - 32) * width;
                    mcal_i2c_data_trasnmit(font[index]);       
            } 
            mcal_i2c_stop_condition();
//            ret |= ecual_oled_display_cursor_set(oled_display, page_num + 1, column); 
        }
...
Std_ReturnType ecual_oled_display_string_write(const oled_display_config_t* oled_display, const uint8_t string[], const uint8_t height, const uint8_t width, uint8_t page, uint8_t column) {
    ...
    if (NULL == oled_display) {
        ...
        int i = 0;
        while('\0' != string[i]) {
            ret = ecual_oled_display_char_write(oled_display, string[i], height, width, page, column);
            i++;
        }
    }
    return ret;

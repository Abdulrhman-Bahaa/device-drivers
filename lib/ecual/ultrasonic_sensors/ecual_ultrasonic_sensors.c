...
Std_ReturnType ecual_us_sensor_init(const us_sensor_config_t* us_sensor) {
    ...
    if (NULL == us_sensor) {
        ...
        ret = mcal_gpio_pin_initialize(&(us_sensor->trigger_pin));
...
Std_ReturnType ecual_us_sensor_invoke_on_range(const us_sensor_config_t* us_sensor, const uint16_t range_start, const uint16_t range_end, 
                                               void (*function_to_invoke)(void)) {
    Std_ReturnType ret = E_OK;
    if((NULL == us_sensor) || (NULL == function_to_invoke) ) {
        ret = E_NOT_OK;
    }
    else {
        uint16_t distance = 0;
...
void us_sensor_app_interrupt_callback(void) { 
    ...
    if (rising_edge == 1) {
        ...
        timer0_reg_value = TCNT0 + (255 * num_of_overflows); 
        num_of_overflows = 0;
        rising_edge = 1;

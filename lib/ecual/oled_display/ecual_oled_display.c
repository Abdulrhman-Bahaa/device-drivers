...
static Std_ReturnType i2c_start_condition(void) {
    Std_ReturnType ret = E_OK;
    ret |= mcal_i2c_start_condition();
    return ret;
}

static Std_ReturnType i2c_stop_condition(void) {
    Std_ReturnType ret = E_OK;
    ret |= mcal_i2c_stop_condition();
    return ret;
}

static Std_ReturnType i2c_byte_transmit(uint8_t data) {
    Std_ReturnType ret = E_OK;
    ret |= mcal_i2c_byte_transmit(data);
    return ret;
}

/**
 * \brief      This function transmits data to the I2C bus
 * \param[in]  address: The I2C address of the slave device
 * \param[in]  data: Pointer to the data to be transmitted
 * \param[in]  size: The size of the data to be transmitted
 * \return     E_OK if the transmission was successful, E_NOT_OK otherwise
 */
static Std_ReturnType i2c_array_transmit(uint8_t address, const uint8_t* data, uint16_t size) {
    Std_ReturnType ret = E_OK;
    if (NULL == data) {
        ret = E_NOT_OK;
    }
    else {
        ret |= mcal_i2c_master_transmit(address, (uint8_t*)data, size);
    }
    return ret;
}
...
Std_ReturnType ecual_oled_display_init(const oled_display_config_t* oled_display) {
    ...
    if (NULL == oled_display) {
        ...
        ret |= i2c_array_transmit((oled_display->oled_display_address), oled_configurations, 19);
    }
    return ret;
}
...
Std_ReturnType ecual_oled_display_addressing_mode(const oled_display_config_t* oled_display, memory_addressing_modes_t mode) {
    ...
    if (NULL == oled_display) {
        ...
        uint8_t addressing_mode_command;

        switch (mode) {
            case HORIZONTAL_ADDRESSING_MODE:
                addressing_mode_command = 0x00;
                break;
            case VERTICAL_ADDRESSING_MODE:
                addressing_mode_command = 0x01;
                break;
            case PAGE_ADDRESSING_MODE:
                addressing_mode_command = 0x02;
                break;
            default:
                return E_NOT_OK; // Invalid enum value
        }

        uint8_t data[] = {COMMAND_STREAM, 0x20, addressing_mode_command};
        ret |= i2c_array_transmit((oled_display->oled_display_address), data, 3);
    }
    return ret;
}
...
Std_ReturnType ecual_oled_display_cursor_set(const oled_display_config_t* oled_display, uint8_t page, uint8_t column) {
    ...
    if ((NULL == oled_display) || (page > 7) || (column > 127)) {
        ...
        uint8_t data[] = {COMMAND_STREAM, OLED_DISPLAY_SET_PAGE_START_ADDRESS_FOR_PAGE_ADDRESSING_MODE | ((uint8_t)page), 
                          OLED_DISPLAY_SET_LOWER_COLUMN_START_ADDRESS_FOR_PAGE_ADDRESSING_MODE | (((uint8_t)column) & 0x0F), 
                          OLED_DISPLAY_SET_HIGHER_COLUMN_START_ADDRESS_FOR_PAGE_ADDRESSING_MODE | ((uint8_t)column) >> 4};
        ret |= i2c_array_transmit((oled_display->oled_display_address), data, 4);
    }
    return ret;
}
...
Std_ReturnType ecual_oled_write_pixel(const oled_display_config_t* oled_display, uint8_t value, uint8_t x, uint8_t y) {
    ...
    if (NULL == oled_display) {
        ...
        ret |= i2c_array_transmit((oled_display->oled_display_address), data, 2);
    }
    return ret;
}
...
Std_ReturnType ecual_oled_display_block(const oled_display_config_t* oled_display, uint8_t data_to_draw, 
    ...
    if (NULL == oled_display) {
        ...
        uint8_t column = 0;
        ret |= ecual_oled_display_addressing_mode(oled_display, PAGE_ADDRESSING_MODE);
        for (uint8_t page_num = start_page; page_num < end_page + 1; page_num++) {
            ...
            ret |= i2c_start_condition();
            ret |= i2c_byte_transmit(oled_display->oled_display_address);
            ret |= i2c_byte_transmit(DATA_STREAM);
            for (uint8_t column_num = start_column; column_num < column + 1; column_num++) {
                ret |= i2c_byte_transmit(data_to_draw); 
            }
            ret |= i2c_stop_condition();      
        }

        // ret |= ecual_oled_display_addressing_mode(oled_display, HORIZONTAL_ADDRESSING_MODE);
...
Std_ReturnType ecual_oled_display_clear(const oled_display_config_t* oled_display) {
    ...
    if (NULL == oled_display) {
        ...
        ret |= ecual_oled_display_block(oled_display, 0x00, 0, 0, 7, 127);
    }
    return ret;
}
